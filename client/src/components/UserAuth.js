...
import { fetchNonce, getUserByToken, signupHandler, verifySignatureHandler } from '../services/user-auth.service';
...
const UserAuth = ({ ethereum, account, setAccount, setError }) => {
    ...
    const signNonce = async ({ nonce, publicAddress }) => {
        ...
        const params = [
            bufferToHex(Buffer.from(msg, 'utf-8')),
            publicAddress,
        ];
    ...
    const handleLogin = async () => {
        try {
            ...
            setError("We see that you didn't connect any account in Metamask.. Please do that and try logging in again..");
        }
    }
    ...
    return (
        <div>
            ...
            {signupFormVisible && (
                <SignupForm signupHandler={
                    ({ publicAddress, name }) => signupHandler({ publicAddress, name })}
                />
            )}
        </div>
    );
