const { getUserData, createUser, verifySignature } = require('../services/smart-contract.service');
...
async function signupController(req, res, next) {
    try {
        ...
        if (userdata.username !== '') {
            throw new AppError({
                statusCode: 400,
                message: 'User already exists. Please login with MetaMask to continue..',
...
async function verifySignatureController(req, res, next) {
    try {
        ...
        const { user, verified } = await verifySignature({ senderAccAddr: publicAddress, signature });
